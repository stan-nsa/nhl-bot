...
async def command_stats_defensemen(message: types.Message):
    await message.answer(f"<b>Статистика защитников:</b>\n{stats.get_stats_defensemen_text(full=False)}", parse_mode="HTML")


async def command_stats_rookie(message: types.Message):
    await message.answer(f"<b>Статистика новичков:</b>\n{stats.get_stats_rookie_text(full=False)}", parse_mode="HTML")
...
def register_handlers_stats(dp : Dispatcher):
    ...
    dp.register_message_handler(command_stats_skater, commands=['stats_defensemen'])
    dp.register_message_handler(command_stats_goalie, commands=['stats_rookie'])
